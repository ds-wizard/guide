...
Each item may be marked with *unsupported metamodel* if the document template is not compatible with the version of |project_name|. Also, *update available* may appear if there is a newer version of the template in the `DSW Registry <https://registry.ds-wizard.org>`__ (and if configured). If not, reffer to the :ref:`FAQ and Deployment Notes<faq-unsupported-metamodel>`.
